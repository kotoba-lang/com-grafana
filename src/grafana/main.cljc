(ns grafana.main
  "Kotodama WASM entrypoint for the Grafana clean-room actor (L4) — Clojure port.
  ...
  facts (`grafana.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "grafana")
(def tier "L4")
...
(def entity-specs
  [{:entity "Dashboard"   :plural "dashboards"   :id-prefix "grafana_das"
    :fields [:uid :title :schemaVersion :version :editable :timezone] :required [:uid]
    :coerce {:schemaVersion :int :version :int :editable :bool} :refs {}}
   {:entity "DataSource"  :plural "datasources"  :id-prefix "grafana_dat"
    :fields [:uid :name :type :access :url :isDefault] :required [:uid]
    :coerce {:isDefault :bool}          :refs {}}
   {:entity "Folder"      :plural "folders"      :id-prefix "grafana_fol"
    :fields [:uid :name :title :resourceVersion :creationTimestamp] :required [:uid :name]
    :coerce {}                       :refs {}}
   {:entity "AlertRule"   :plural "alertrules"   :id-prefix "grafana_ale"
    :fields [:uid :title :condition :noDataState :execErrState :isPaused] :required [:uid :title]
    :coerce {:isPaused :bool}        :refs {}}
   {:entity "User"        :plural "users"        :id-prefix "grafana_use"
    :fields [:email :name :login :isGrafanaAdmin :isDisabled] :required [:email]
    :coerce {:isGrafanaAdmin :bool :isDisabled :bool} :refs {}}
   {:entity "Organization" :plural "organizations" :id-prefix "grafana_org"
    :fields [:name :address]          :required [:name]
    :coerce {}                     :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"grafana.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "grafana-compat" :tier tier :entities entities} 200])
...
(defn start! [] :grafana-compat/ready)
